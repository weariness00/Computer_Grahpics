...
mat4 CoorRotate(vec3 quaternion)
{
	//mat4 ro = mat4(1.0);

	//for (int i = 0; i < 4; i++)
	//	ro[i].w = 0;
	//ro[3].x = ro[3].y = ro[3].z = 0;
	//ro[3].w = 1;

	float x, y, z;
	x = radians(quaternion.x);
	y = radians(quaternion.y);
	z = radians(quaternion.z);

	//ro[0].x = cos(z) * cos(y);
	//ro[0].y = sin(z) * cos(x) + cos(z) * sin(y) * sin(x);
	//ro[0].z = sin(z) * sin(x) - cos(z) * sin(y) * sin(x);

	mat4 rx, ry, rz;
	rx = ry = rz = mat4(0);

	rx[0].x = 1;
	rx[1].y = cos(x); rx[1].z = sin(x);
	rx[2].y = -sin(x); rx[2].z = cos(x);
	rx[3].w = 1;

	ry[0].x = cos(y); ry[0].z = -sin(y);
	ry[1].y = 1;
	ry[2].x = sin(y); ry[2].z = cos(y);
	ry[3].w = 1;

	rz[0].x = cos(z); rz[0].y = sin(z);
	rz[1].x = -sin(z); rz[1].y = cos(z);
	rz[2].z = 1;
	rz[3].w = 1;
