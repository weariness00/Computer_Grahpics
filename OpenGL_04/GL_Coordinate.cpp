...
void GL_Coordinate::Init()
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		lineObject[i].transform.worldPivot.y = 0.5;
	}
	lineObject[0].color.R = 1;
	lineObject[1].color.G = 1;
	lineObject[2].color.B = 1;

	lineObject[0].transform.worldRotation += -30;
	lineObject[1].transform.worldRotation += 30;

	lineObject[2].transform.worldRotation.z = 90;
}